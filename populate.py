"""Populate the database with realistic sample data. Run: python populate.py"""
import sqlite3
import random
import os
...
def populate():
    ...
    for row in MEDICINES:
        existing = db.execute('SELECT medicine_id FROM medicines WHERE medicine_name=?',(row[0],)).fetchone()
        if existing:
            med_ids.append(existing['medicine_id'])
        else:
            cur = db.execute('INSERT INTO medicines (medicine_name,manufacturer,category,mrp,discount_percentage,low_stock_threshold) VALUES (?,?,?,?,?,?)',row)
            med_ids.append(cur.lastrowid)

    batch_counter = 1
    for i, mid in enumerate(med_ids):
        for b in range(random.randint(2,3)):
            if b == 0 and i % 6 == 0: exp_days = random.randint(15,25)
            elif b == 0 and i % 6 == 1: exp_days = random.randint(35,55)
            elif b == 0 and i % 6 == 2: exp_days = random.randint(65,85)
            else: exp_days = random.randint(120,540)
            qty = random.randint(20,120)
            mfg = future_date(-(exp_days+random.randint(180,365)))
            exp = future_date(exp_days)
            db.execute('INSERT INTO batches (batch_no,medicine_id,manufacturing_date,expiry_date,quantity) VALUES (?,?,?,?,?)',
    ...
    if db.execute('SELECT COUNT(*) as c FROM bills').fetchone()['c'] == 0:
        for _ in range(55):
            cid = random.choice(cust_ids)
            bill_date = random_date(90)
            chosen_meds = random.sample(list(zip(med_ids,MEDICINES)),random.randint(1,4))
            subtotal = 0.0
            bill_items = []
            valid = True
            for mid, med_data in chosen_meds:
                qty = random.randint(1,3)
                avail = db.execute('SELECT COALESCE(SUM(quantity),0) as s FROM batches WHERE medicine_id=? AND quantity>0',(mid,)).fetchone()['s']
                if avail < qty: valid = False; break
                unit_price = round(med_data[3]*(1-med_data[4]/100),2)
                subtotal += unit_price*qty
                bill_items.append((mid,qty,unit_price))
            if not valid or not bill_items: continue
            gst = round(subtotal*0.18,2)
            cur = db.execute('INSERT INTO bills (customer_id,date,subtotal,gst,final_amount) VALUES (?,?,?,?,?)',
                             (cid,bill_date,round(subtotal,2),gst,round(subtotal+gst,2)))
            bill_id = cur.lastrowid
            for mid, qty, unit_price in bill_items:
                db.execute('INSERT INTO bill_items (bill_id,medicine_id,quantity,unit_price) VALUES (?,?,?,?)',(bill_id,mid,qty,unit_price))
                remaining = qty
                for batch in db.execute('SELECT * FROM batches WHERE medicine_id=? AND quantity>0 ORDER BY expiry_date ASC',(mid,)).fetchall():
    ...
    db.commit()
    db.close()
    print("Done! 50 medicines, 50 customers, ~55 bills. Login: admin/admin123  staff/staff123")
