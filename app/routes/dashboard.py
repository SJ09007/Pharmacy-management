...
def index():
    ...
    low_stock = db.execute(
        '''SELECT m.medicine_name, m.low_stock_threshold,
           ...
           GROUP BY m.medicine_id HAVING stock < m.low_stock_threshold'''
    ).fetchall()
...
def daily_revenue():
    db = get_db()
    rows = db.execute("SELECT date, SUM(final_amount) as revenue FROM bills GROUP BY date ORDER BY date DESC LIMIT 30").fetchall()
    db.close()
...
def monthly_revenue():
    db = get_db()
    rows = db.execute("SELECT strftime('%Y-%m', date) as month, SUM(final_amount) as revenue FROM bills GROUP BY month ORDER BY month DESC LIMIT 12").fetchall()
    db.close()
